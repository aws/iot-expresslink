...
class OpenSearchPatternStack(Stack):
    ...
    # Default values for OpenSearch domain CapacityConfig
    opensearch_domain_data_nodes = 3
    opensearch_domain_data_node_instance_type = "t3.small.search"
    opensearch_domain_master_nodes = ""
    opensearch_domain_warm_nodes = ""
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        capacity=opensearch.CapacityConfig(
            data_nodes=self.opensearch_domain_data_nodes,
            data_node_instance_type=self.opensearch_domain_data_node_instance_type
        ), 
        enforce_https=True,
    ...
    def performInputValidation(self):
        ...
        self.validateCapacityConfig(self.node.try_get_context("opensearch_domain_capacity_config"))
    ...
    def validateOpensearchDomainName(self, input):
        if not input:
            ...
            checkInputLength(self, 3, 28, input, "OpenSearch domain")
            checkInputPattern(self, r'^[a-z0-9-]+$', input, "OpenSearch domain")
    ...
    def validateCognitoUserPoolName(self, input):
        if not input:
            ...
            checkInputLength(self, 1, 128, input, "Cognito UserPool")
    ...
    def validateCognitoUserPoolDomainName(self, input):
        if not input:
            ...
            checkInputLength(self, 1, 63, input, "Cognito UserPool domain")
            checkInputPattern(self, r'^[a-z0-9](?:[a-z0-9\-]{0,61}[a-z0-9])?$', input, "Cognito UserPool domain")
    ...
    def validateCognitoIdentityPoolName(self, input):
        if not input:
            ...
            checkInputLength(self, 1, 128, input, "Cognito IdentityPool")
            checkInputPattern(self, r'^[\w\s+=,.@-]+$', input, "Cognito IdentityPool")
    ...
    def validateCapacityConfig(self, input):
        if input:
            self.opensearch_domain_data_nodes = input["opensearch_domain_data_nodes"] if "opensearch_domain_data_nodes" in input else 3
            self.opensearch_domain_data_node_instance_type = input["opensearch_domain_data_node_instance_type"] if "opensearch_domain_data_node_instance_type" in input else "t3.small.search"
            self.opensearch_domain_master_nodes = input["opensearch_domain_master_nodes"] if "opensearch_domain_master_nodes" in input else ""
            self.opensearch_domain_warm_nodes = input["opensearch_domain_warm_nodes"] if "opensearch_domain_warm_nodes" in input else ""