...
class KinesisPatternStack(Stack):
    ...
    def validateBucketName(self, input):
        ...
        elif len(input) < 3 or len(input) > 63:
            raise WrongLengthForInput("Bucket names must be between 3 (min) and 63 (max) characters long.")
        elif not re.match(r'(?!(^xn--|-s3alias$))^[a-z0-9][a-z0-9-]{1,61}[a-z0-9]$', input):
            raise WrongFormattedInput("Wrong formatted input for s3 bucket name. Please refer to the README.md for more information about bucket naming requirements.")     
        else:
            return     
    ...
    def validateDeliveryStreamName(self, input):
        if not input:
            ...
            if type(input) != str:
                ...
                raise WrongFormattedInput("String format error for delivery stream name:  Acceptable characters are uppercase and lowercase letters, numbers, underscores, hyphens, and periods.")     
    ...
    def validateGlueDBName(self, input):
        if not input:
            ...
            raise WrongLengthForInput("The length of the Glue database name string should not exceed 255 characters.")
        else:
            return      
